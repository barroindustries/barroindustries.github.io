...
## 🆕 SESSION 2026-08-24 — PERF-WAVE1: lazy screens, deploy-diff SW, read caching

Spec: [`specs/PERF-WAVE1-SPEC-2026-08-24.md`](specs/PERF-WAVE1-SPEC-2026-08-24.md). Boot JS
5.0 MB → 2.17 MB (gzip 1468 → 680 KB): 25 dept/feature files now lazy-load per page via
`PAGE_SCRIPTS`/`ensurePage` (config.js) awaited in async `navigateTo`. sw.js installs by
diffing `precache-manifest.json` (generated by the pre-commit hook) and copies unchanged
files forward — a deploy re-downloads only what changed. Hot Firestore reads routed through
`dbCachedGet` with choke-point invalidation (`approvals-pending:*`, `tasks-mine-*`,
`ca-mine-*` via the `ca-pending` alias); Analytics Chart.js leaks fixed; long lists capped
at 150 + Show-all. New rules: new screen ⇒ PRECACHE + PAGE_SCRIPTS entry; never commit
`--no-verify` (CI invariant checks manifest vs HEAD). Wave-2 backlog: split hr.js payslip
renderer, chat.js, dashboards.js; slim production.js out of 8 bundles; Lucide subset; CSS
dedupe. Repo also reorganized: root notes merged into `docs/` + `specs/` (index:
[`docs/README.md`](docs/README.md)); scratch quarantined in gitignored `dev/_attic/`.
