...
function cirIpBucketRef(db, scope, ipKey) {
  return db.collection('cir_ratelimit').doc(portalSha256Hex(scope + '|' + ipKey));
}

/**
 * The rate-limit bucket key for this caller, or a FAIL-CLOSED throw.
 *
 * `portalClientIp()` returns portal-core's `{ip, family, trusted, source}` —
 * an OBJECT, deliberately, so that an untrusted address cannot be used as if
 * it were a real one. Passing it straight into a doc-id string (which is what
 * `scope + '|' + ip` did while the anti-spoof change and this feature were in
 * flight at the same time) stringifies to the literal '[object Object]',
 * collapsing EVERY caller into one shared bucket — 20 starts/hour, 80
 * photos/hour and 5 submits/DAY for the whole internet combined, i.e. the
 * first five submitters lock out everyone else for 24 hours.
 *
 * The denial below is byte-identical to cirRateLimit's own wording, so a
 * caller can't tell "no trustworthy IP" from "actually rate limited".
 */
function cirRequireIpKey(db, ipInfo) {
  const key = portal.rateLimitIpKey(ipInfo);
  if (!key) {
    console.error('[cir] no trustworthy client IP — denying (source=%s)', ipInfo && ipInfo.source);
    cirLogAbuse(db, 'rate_limited', '', { reason: 'untrusted_ip' }).catch(() => {});
    throw new functions.https.HttpsError('resource-exhausted', 'Too many requests from this connection. Try again in 15 minutes.');
  }
  return key;
}
...
exports.cirStartDraft = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const ipInfo = portalClientIp(context);
    const ipKey = cirRequireIpKey(db, ipInfo);
    const ip = ipInfo.ip;
    const userAgent = (context.rawRequest && context.rawRequest.headers && context.rawRequest.headers['user-agent']) || '';

    await cirRateLimit(db, [
      { ref: cirIpBucketRef(db, 'start', ipKey), cfg: cir.RATE.start },
...
exports.cirUploadPhoto = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    await cirRateLimit(db, [
      { ref: cirIpBucketRef(db, 'photo', ipKey), cfg: cir.RATE.photo },
...
exports.cirRemovePhoto = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    await cirRateLimit(db, [{ ref: cirIpBucketRef(db, 'photo', ipKey), cfg: cir.RATE.photo }], nowMs);
...
exports.cirSubmit = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    await cirRateLimit(db, [
      { ref: cirIpBucketRef(db, 'submit', ipKey), cfg: cir.RATE.submit },
