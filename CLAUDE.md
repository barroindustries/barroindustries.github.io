...
- **Deploy the app:** `git push origin master` (or `release.sh push`). Auto-deploys to **GitHub Pages** at `barroindustries.github.io` → custom domain `barroindustries.com` (old `barroindustries-operatingsystem.ravenmails.com` redirects via the `ops-legacy-redirect` repo) (remote: `https://github.com/barroindustries/barroindustries.github.io.git`). Branch `master`. **Not Netlify.**
