...
// mkEntry() returns a Ledger.post-shaped entry (ref/date/kind/... /extra).
async function _syncLedgerLegViaUpsert(ref, amount, mkEntry) {
  if (amount > 0) {
    await window.Ledger.upsertByRef(ref, () => mkEntry());
  } else {
    await _deleteLedgerByRef(ref);
  }
}

// backfillLedgerFromJournals + backfillPayrollLedger moved to js/migrations.js (v13 Phase 37)
      ...
      : `<div class="card"><div class="table-wrap"><table class="data-table table-cards">
          ...
          <tbody>${quotes.map(q=>{
            ...
            return `<tr class="bsq-row">
              <td class="tc-avatar"><code>${escHtml(q.quoteNumber||q.id.slice(-8))}</code></td>
