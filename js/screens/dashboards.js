    ...
    wrap.innerHTML = `
      ...
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="card-body">
          ...
          ${usingEstimate ? `<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">${emojiIcon('🧮',12)} Standard-cost estimate (no variable costs classified in the ledger yet): materials ₱${fmt(matSpend)} × ${stdCostMult.toFixed(2)} (waste ${Math.round(wastePct*100)}% + labor ${Math.round(laborPctOfMat*100)}% + consumables ${Math.round(consumablesPct*100)}% of materials) → est. variable ₱${fmt(estVariable)}</div>` : ''}
